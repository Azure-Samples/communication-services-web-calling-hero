...
import {
    callAdded, callRemoved, setCallState, setParticipants, setCallAgent,
    setServerCallId, startRecording, /*pauseRecording, resumeRecording,*/
    stopRecording, resetRecordingStatus, recordingError
} from './actions/calls';
...
export const startRecord = () => {
    return async (dispatch: Dispatch, getState: () => State): Promise<void> => {
        ...
        if (state.calls !== undefined && state.calls.serverCallId) {
            ...
            if (response && !response.message && (response.recordingId !== '')) {
                dispatch(startRecording(response.recordingId.toString(), Constants.STARTED));
            } else {
...
/*export const pauseRecord = () => {
    return async (dispatch: Dispatch, getState: () => State): Promise<void> => {
        ...
        if (state.calls !== undefined && state.calls.serverCallId && state.calls.recordingId !== undefined) {
            const response: RecordingActionResponse = await utils.pauseRecording(state.calls.serverCallId, state.calls.recordingId);
            if (response && !response.message) {
                dispatch(pauseRecording(Constants.PAUSED));
            } else {
                console.error(response.message);
            }
        }
...
/*export const resumeRecord = () => {
    return async (dispatch: Dispatch, getState: () => State): Promise<void> => {
        let state = getState();
        if (state.calls !== undefined && state.calls.serverCallId && state.calls.recordingId !== undefined) {
            const response: RecordingActionResponse = await utils.resumeRecording(state.calls.serverCallId, state.calls.recordingId);
            if (response && !response.message) {
                dispatch(resumeRecording(Constants.RESUMED));
            } else {
                console.error(response.message);
            }
        }
        ...
        callAgent.on('callsUpdated', (e: { added: Call[]; removed: Call[] }): void => {
            e.added.forEach((addedCall) => {
                ...
                const isRecordingActiveChangedHandler = () => {
                    ...
                    else if (!(callRecordingApi.isRecordingActive)) {
                        if (getState().calls.recordingId === '' || getState().calls.recordingStatus === Constants.STARTED) {
                            dispatch(stopRecording(Constants.STOPPED));
                        }
                    }
                };
